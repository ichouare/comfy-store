...
const SaleProduct = ({data}) => {
  ...
  return( 
    <div className='w-full h-32  md:h-40 rounded-lg justify-center items-center  flex gap-4 border  p-2 "  '>
      <div className='w-[40%] h-full bg-product-bg  dark:bg-white grid place-content-center    rounded-[14px]  '>
          <img src={image} alt='' className=' h-[100%] scale-75 object-fill'/>
        </div>
        ...
        <p className='text-start '>$ {price * quantity}</p>
            <div className='flex items-center justify-between p-2 bg-zinc-100 w-32  rounded-2xl min-h-8 h-8 text-base self-end'>
                <span className='text-lg cursor-pointer' onClick={() => {
                  ...
                  setCart( prev => ({...prev, 
                    numItemsInCart : prev.numItemsInCart > 0 ? prev.numItemsInCart-- : 0, 
                    cartTotal : prev.cartTotal - (price * Qt)  || 0,
                  }))
                ...
                <span className='text-lg cursor-pointer' onClick={() => {
                  ...
                  setCart( prev => ({...prev, 
                    ...
                    cartTotal : prev.cartTotal + (price * Qt),
                  }))
                }}   > <FiPlus /></span>
            </div>
      </div>
...
console.log(cart)
   return (
      <div className='w-[100%]   max-sm:w-full  min-h-screen h-full  px-6 flex flex-col items-start  p-4'>
       ...
       <div className='w-full  h-full  md:pt-12 grid gap-y-4  gap-4  grid-cols-1   place-items-start  py-[10px] ' >
